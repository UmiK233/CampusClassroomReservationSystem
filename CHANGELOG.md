...
## [v0.20.0] - 2026-05-10
### 🔄 更新
* 维护创建逻辑支持“清退冲突预约”开关：默认仍会在维护时间段内存在预约时拒绝创建维护，管理员显式开启后，系统会先创建维护，再取消冲突预约并通知相关用户。
* 后台“维护管理”页面新增“清退维护时间段内的冲突预约”选项和二次确认提示，便于处理临时维修、考试占用等需要强制腾退资源的场景。
* 维护创建审计日志补充 `clear_conflicts=true` 标记，便于区分普通维护创建和带清退的维护创建。

### ✅ 测试
* 新增 `MaintenanceServiceImplTest`，覆盖“默认拒绝有冲突预约的维护创建”和“开启清退后创建维护并取消冲突预约”两个分支。
...
## [v0.19.0] - 2026-05-04
### 文档补全
* 新增 `部署文档.md`，完整整理环境要求、MySQL/Redis/Mail 配置、初始化 SQL、升级 SQL、前后端启动步骤、Docker 部署步骤与默认演示账号。
* 新增 `系统设计说明.md`，整理系统总体架构、模块划分、数据库 E-R、核心表结构、接口分组设计，以及认证流程、设备会话流程、候补补位流程。

### 修改过程
* 梳理了当前项目的实际运行依赖与部署入口，将本地启动、Docker 启动和老库升级路径统一写入独立文档。
* 将认证、设备会话、预约、签到、候补、通知、后台治理等核心设计抽成可直接用于论文或答辩材料的系统说明。
