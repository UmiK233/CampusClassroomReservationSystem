...
* 新增管理员强制下线功能：后台用户管理页可对非管理员账号执行“强制下线”，后端会递增目标用户的 `tokenVersion` 并撤销其全部设备会话，使该用户的 `accessToken` 与 `refreshToken` 立即失效。
* 管理员用户列表新增在线状态字段，前端仅对在线的非管理员账号展示“强制下线”入口；同时修正无匹配路由时的异常映射，将 `NoResourceFoundException` 从 `500` 调整为 `404`。
* 新增管理员操作审计：后端增加 `admin_audit_log` 表、审计查询接口与日志写入服务，覆盖用户状态修改、强制下线、预约取消、配置修改、教室创建/更新、维护创建/取消等关键管理动作；后台新增“操作日志”页面用于检索和查看审计记录。
